...
class GhostWidgetSmallProvider : AppWidgetProvider() {
    ...
    private fun showLoading(ctx: Context, mgr: AppWidgetManager, id: Int) {
        ...
        views.setTextViewText(R.id.gs_title_0, "Loading...")
        views.setTextViewText(R.id.gs_title_1, "")
        views.setTextViewText(R.id.gs_title_2, "")
        mgr.updateAppWidget(id, views)
    }

    private fun showError(ctx: Context, mgr: AppWidgetManager, id: Int) {
        val views = RemoteViews(ctx.packageName, R.layout.ghost_widget_small)
        views.setTextViewText(R.id.gs_title_0, "Tap to refresh")
        views.setTextViewText(R.id.gs_num_0, "!")
        val intent = Intent(ctx, GhostWidgetSmallProvider::class.java).apply { action = ACTION_REFRESH }
        val pi = PendingIntent.getBroadcast(ctx, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        views.setOnClickPendingIntent(R.id.gs_row_0, pi)
        mgr.updateAppWidget(id, views)
    }
    ...
    private fun updateWidget(ctx: Context, mgr: AppWidgetManager, id: Int, stories: List<GhostWidgetProvider.Story>, prices: List<CoinPrice>) {
        ...
        ctx.packageManager.getLaunchIntentForPackage(ctx.packageName)?.let {
            val pi = PendingIntent.getActivity(ctx, 1, it, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
            views.setOnClickPendingIntent(R.id.gs_header, pi)
        }

        val rowIds   = intArrayOf(R.id.gs_row_0, R.id.gs_row_1, R.id.gs_row_2)
        val numIds   = intArrayOf(R.id.gs_num_0, R.id.gs_num_1, R.id.gs_num_2)
        val titleIds = intArrayOf(R.id.gs_title_0, R.id.gs_title_1, R.id.gs_title_2)
        val timeIds  = intArrayOf(R.id.gs_time_0, R.id.gs_time_1, R.id.gs_time_2)
        ...
        for (i in 0 until 3) {
            if (i < stories.size) {
                ...
                if (s.link.isNotEmpty()) {
                    val pi = PendingIntent.getActivity(ctx, 400 + i,
    ...
    private fun timeAgo(pubDate: String): String {
        ...
        return try {
            ...
            var date: Date? = null
            for (fmt in formats) {
                try { date = SimpleDateFormat(fmt, Locale.US).also { it.timeZone = TimeZone.getTimeZone("UTC") }.parse(pubDate); if (date != null) break } catch (_: Exception) {}
            }
            val d = date ?: return ""
